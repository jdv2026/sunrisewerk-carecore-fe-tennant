...
import { fetchUserResponse, OnboardingService, UserAttributes } from '../../core/onboarding/onboarding.service';
...
export class OnboardingComponent implements OnInit, OnDestroy {
    ...
    steps: Step[] = [
        ...
        { number: 3, icon: 'link',           label: 'Your Subdomain' },
        { number: 4, icon: 'rate_review',    label: 'Review'         },
        { number: 5, icon: 'check_circle',   label: "You're All Set" },
    ];
	...
	async ngOnInit(): Promise<void> {
        ...
        try {
            const attrs: fetchUserResponse = await this.onboardingService.fetchUserAttributes(email);
            this.clinicDomain.set(attrs.data.clinic_domain_name ?? '');
            this.clinicForm.patchValue({ clinicName: attrs.data.clinic_name ?? '' });
	...
	async confirmSubmit() {
        ...
        try {
            ...
            const slug = this.subdomainForm.get('slug')!.value;
    ...
    get slug() { return this.subdomainForm.get('slug')!; }
    ...
    async checkSlug(): Promise<void> {
        if (this.slug.invalid) return;
        this.slugUnchecked.set(false);
        this.slugChecking.set(true);
        this.slugAvailable.set(null);
        try {
            const res = await fetch(`${environment.backendApi}domain-availability?slug=${encodeURIComponent(this.slug.value)}`);
            const json = await res.json();
            this.slugAvailable.set(json.available === true);
        } catch {
            this.slugAvailable.set(null);
        } finally {
    ...
    onSlugInput() {
        const raw = this.slug.value ?? '';
        const cleaned = raw.toLowerCase().replace(/[^a-z0-9\-]/g, '');
        if (raw !== cleaned) this.slug.setValue(cleaned, { emitEvent: false });
        this.slugAvailable.set(null);
        this.slugUnchecked.set(false);
    }
    ...
    next() {
        const form = this.currentStep() === 1 ? this.profileForm
                   ...
                   : this.currentStep() === 3 ? this.subdomainForm
                   : null;
        ...
        if (this.currentStep() === 3) {
            if (this.slugAvailable() === null) {
                this.slugUnchecked.set(true);
                return;
            }
            if (!this.slugAvailable()) return;
        }
    ...
    get clinicLoginUrl() {
        return `https://${this.clinicDomain()}.sunrisewerk.com`;
    }
